...
void DesCurtailed(J, pi0, pi1, alpha, beta, Nmin, Nmax, futility, efficacy, optimality, equal_n, ensign, summary, plot, xopt)
{
  ...
  thetaEog = thetaE
  if (J > 1) {
    thetaF[mm_which(thetaF :== 0)] = J(1, length(mm_which(thetaF :== 0)), 1e-10)
    thetaE[mm_which(thetaE :== 1)] = J(1, length(mm_which(thetaE :== 1)), 1 - 1e-10)
  }
  else {
    if (thetaF == 0) {
	  thetaF = 1e-10
  ...
  if (des ~= .) {
	...
	if (J > 1) {
	  a[mm_which(a :== -1)] = J(1, length(mm_which(a :== -1)), .)
      r[mm_which(r :== sum(n) + 1)] = J(1, length(mm_which(r :== sum(n) + 1)), .)
	}
	a_curt = J(1, sum(n), .)
	r_curt = J(1, sum(n), .)
	n_curt = J(1, sum(n), 1)
    ...
    if (plot ~= "") {
	  ...
	  for (j = 2; j <= J_curt; j++) {
	    ...
	    for (i = 1; i <= rows(states_j); i++) {
		  ...
		  else if (states_j[i, 1] >= r_curt[j] & states_j[i, 2] == sum(n_curt[1::j])) {
		    outcome[i] = 2
		  }
	    }
  ...
  st_matrix("beta", beta)
  st_matrix("thetaF", thetaFog)
  st_matrix("thetaE", thetaEog)
  st_matrix("Nmin", Nmin)
  st_matrix("Nmax", Nmax)
